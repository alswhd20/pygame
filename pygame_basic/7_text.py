...
#배경 이미지 불러오기
background = pygame.image.load("C:\\python\\pygame\\pygame_basic\\background.png")

# 캐릭터 스프라이트 불러오기
character =pygame.image.load("C:\\python\\pygame\\pygame_basic\\character.png")
...
# 적 enemy 캐릭터
enemy =pygame.image.load("C:\\python\\pygame\\pygame_basic\\enemy.png")
...
#캐릭터ㅏ 100만큼 이동을 해야함 
#10 fps : 1초 동안에 10번동작 -> 1번에 10만큼 10*10 =100
#20 fps : 1초 동안에 20번동작 -> 1번 5 5 * 20 =100
    print ("fps:" + str(clock.get_fps()))

    for event in pygame.event.get(): # 어떤 이벤트가 발생하였는지
        if event.type == pygame.QUIT: #창을 닫히는 이벤트가 발생할떄
            running = False #게임이 진행중이 아님 
        
        if event.type == pygame.KEYDOWN: #키 이벤트 확인
    ...
    #가로 경계값 처리 (전체크기의 자기 캐릭터 이미지를 빼줘야지 화면시야에 벗아나지않음)
    ...
    #경과 시간(ms)을 1000으로 나누어서 초(s) 단위로표시 

    timer = game_font.render(str(int(total_time-elapsed_time)),True,(255,255,255))
    screen.blit(timer, (10, 10)) #배경 그리기
